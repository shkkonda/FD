...
map("state", regions=states$name[m], proj="albers", param=c(39,45), fill=TRUE, col=statecols, border=NA, resolution=1)
...
for (mon in months) {
	...
	map("state", regions=states.mon$name[m], proj="albers", param=c(39,45), fill=TRUE, col=statecols.mon, border=NA, resolution=0)
